...
const QuizzTable = () => {
  const { data: quizes, isLoading, isError } = useGetQuizesListQuery();
  ...
  const handleEdit = (id) => {
    const quizToEdit = quizes.find((quiz) => quiz.id === id);
    setShowModal(true);
  ...
  if (!isLoading && !isError && quizes?.length > 0) {
    content = (
      <>
        {quizes.map((quiz) => (
          <tr key={quiz.id}>
            <td className="table-td">{limitText(quiz.question, 80)}</td>
            <td className="table-td">{limitText(quiz.video_title, 140)} </td>
            <td className="table-td">{quiz.totalMark}</td>
